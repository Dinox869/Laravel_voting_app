<div class="idea-and-buttons container">
                       ...
                       @if(@auth()->check() && auth()->user()->isAdmin())
                        <div class="relative">
                            ...
                            <ul x-show="isOpen" @clik.away="isOpen = false" x-show.transition.origin.top.left.duration.500ms="isOpen" x-cloak @keydown.escape.window="isOpen = false" class="w-44 absolute text-left z-10 md:ml-9 top-8 md:top-6 right-0 md:left-0 font-semibold bg-white shadow-dialog rounded-xl py-3">
                                ...
                                <li><a @click="
                                isOpen = false
                                $dispatch('custom-delete')" href="#" class="px-5 py-3 transition duration-150 ease-in hover:bg-gray-100 block">Delete Idea</a></li>
    ...
    <div class="button-container flex items-center justify-between mt-6">
        <div class="md:ml-6 flex flex-col md:flex-row items-center space-x-4">
            <div x-init="window.livewire.on('postedComment',()=>{ isOpen = false })" x-data="{ isOpen : false}" class="relative">
                ...
                <div  x-show.transition.origin.top.right.duration.500ms="isOpen" @click.away="isOpen = false" @keydown.escape.window="isOpen = false" class=" absolute z-10 md:w-104 w-64 text-left font-semibold rounded-xl mt-2 shadow-dialog bg-white">
                    <form wire:submit.prevent="postComment" action="#" method="POST" class="space-y-4 px-4 py-6">
                      @csrf
                        <div>
                            <textarea wire:model="body" placeholder="Go ahead don't be shy. Share your thoughts..." name="body" id="body" cols="30" rows="4" class="py-2 border-none text-sm w-full rounded-xl placeholder-gray-900 bg-gray-100" required></textarea>
                            @error('body')
                                   <span class="text-red text-xxs font-bold">
                                        {{$message}}
                                   </span>
                            @enderror
                        </div>
                        <div class="flex md:space-x-3 flex-col md:flex-row items-center">
                            <button type="submit" class="flex text-white justify-start items-center w-full md:w-1/2 h-11 text-sm bg-blue font-semibold rounded-xl border border-blue hover:bg-blue-hover transition duration-150 ease-in px-6 py-3">
                                Post Comment
                            </button>
